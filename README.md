...
- Copy all the files from coinor-symphony to SYMPHONY-5.6.17 (for example)
- In SYMPHONY-5.6.17 create a build directory (mkdir build)
- In SYMPHONY-5.6.17/build, launch 'cmake -DCMAKE_INSTALL_PREFIX=/opt/symphony-5.6.17 -DCMAKE_BUILD_TYPE=RELEASE ..'
- In SYMPHONY-5.6.17/build, launch 'cmake-gui .' to fine tune the configuration
- Make the libraries: make
- Install the libraries: sudo make install

An example:
```
$ wget https://www.coin-or.org/download/source/SYMPHONY/SYMPHONY-5.6.17.tgz
$ tar xvfz SYMPHONY-5.6.17.tgz
$ git clone https://github.com/ycollet/coinor-symphony.git
$ cp -r coinor-symphony/* SYMPHONY-5.6.17/
$ cd SYMPHONY-5.6.17
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=/opt/symphony-5.6.17 -DCMAKE_BUILD_TYPE=RELEASE ..
...
- Copy all the files from coinor-symphony to SYMPHONY-5.6.17 (for example)
- In SYMPHONY-5.6.17 create a build directory (mkdir build)
- In SYMPHONY-5.6.17/build, launch 'cmake -DCMAKE_INSTALL_PREFIX=/opt/symphony-5.6.17 -DCMAKE_BUILD_TYPE=RELEASE -G "NMake Makefiles" ..'
- In SYMPHONY-5.6.17/build, launch 'cmake-gui .' to fine tune the configuration
...
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=~/symphony-bin -DCMAKE_BUILD_TYPE=DEBUG -DCMAKE_CXX_COMPILER=/usr/bin/clang -DCMAKE_CXX_FLAGS="-fsanitize=memory" ..
# or
$ cmake -DCMAKE_INSTALL_PREFIX=~/symphony-bin -DCMAKE_BUILD_TYPE=DEBUG -DCMAKE_CXX_COMPILER=/usr/bin/clang++ -DCMAKE_C_COMPILER=/usr/bin/clang \ 
...
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=~/symphony-bin -DCMAKE_BUILD_TYPE=DEBUG -DCMAKE_CXX_COMPILER=/opt/intel/bin/icpc -DCMAKE_C_COMPILER=/opt/intel/bin/icc ..
$ make
```
...
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=d:/symphony-bin -DCMAKE_BUILD_TYPE=DEBUG -G "NMake Makefiles" ..
$ nmake
```
...
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=/home/me/symphony-bin -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_TOOLCHAIN_FILE=cmake/mingw64-toolchain.cmake ..
$ make
```
...
$ cp -r ../coinor-symphony/* .
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=/opt/symphony-devel -DCMAKE_BUILD_TYPE=RELEASE ..
$ make
...
$ coinbrew/coinbrew fetch --git --main-proj="SYMPHONY"
$ cp -r ../coinor-symphony/* SYMPHONY/
$ cd SYMPHONY
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=/opt/symphony-devel -DCMAKE_BUILD_TYPE=RELEASE ..
...
$ git clone https://github.com/coin-or/SYMPHONY
$ cd SYMPHONY
